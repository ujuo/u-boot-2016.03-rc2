...
{
	return fdt_setprop(fdt, chosenoff, "stdout-path",
			      OF_STDOUT_PATH, strlen(OF_STDOUT_PATH) + 1); //linux,?
}
...
static int fdt_fixup_stdout(void *fdt, int chosenoff)
{
	...
	/* ksw : it is very odd that Serial driver wants serialX:baud format 
	  but this path returns with "/soc@XXX/seria@XXXX".
	  so I decide to switch to sername instead of path. */
	//memcpy(tmp, path, len);
	len = strlen(sername);
	memcpy(tmp, sername, len);

	err = fdt_setprop(fdt, chosenoff, "stdout-path", tmp, len); //linux,stdout-path
	if (err < 0)
