...
static int asix_read_mac_common(struct ueth_data *dev,struct asix_private *priv, uint8_t *enetaddr)
{
	u8 buf[ETH_ALEN];
...
static int asix_write_mac_common(struct ueth_data *dev, uint8_t *enetaddr)
{
	int ret;
...
static int asix_basic_reset(struct ueth_data *dev, struct asix_private *dev_priv)
{
	u8 buf[5];
	u16 *tmp16;
	u8 *tmp;
...
static int asix_init_common(struct ueth_data *dev, struct asix_private *dev_priv, uint8_t *enetaddr)
{
...
static int asix_init(struct eth_device *eth, bd_t *bd)
{
	...
	return asix_init_common(dev, dev_priv, NULL);
}
...
static int asix_read_mac(struct eth_device *eth)
{
	...
	return asix_read_mac_common(dev, dev->dev_priv, eth->enetaddr);
}
...
static int asix_write_mac(struct eth_device *eth)
{
	...
	return asix_write_mac_common(dev, eth->enetaddr);
}
...
static int asix_eth_start(struct udevice *dev)
{
	...
	return asix_init_common(&priv->ueth, priv, pdata->enetaddr);
}

static void asix_eth_stop(struct udevice *dev)
{
	debug("** %s()\n", __func__);
}

static int asix_eth_send(struct udevice *dev, void *packet, int length)
{
	struct asix_private *priv = dev_get_priv(dev);
...
static int asix_eth_recv(struct udevice *dev, int flags, uchar **packetp)
{
	struct asix_private *priv = dev_get_priv(dev);
...
static int asix_write_hwaddr(struct udevice *dev)
{
	struct eth_pdata *pdata = dev_get_platdata(dev);
	struct asix_private *priv = dev_get_priv(dev);

	return asix_write_mac_common(&priv->ueth, pdata->enetaddr);
}
...
static int asix_eth_probe(struct udevice *dev)
{
	...
	ret = usb_ether_register(dev, ss, AX_RX_URB_SIZE); //? Hunch.
	debug("ASIX88179 register ret=%d\n", ret);
	if (ret)
		return ret;
	...
	debug("ASIX88179 get mac addr ret=%d\n", ret);
	if (ret)
		goto err;
	debug("MAC %pM\n", pdata->enetaddr);
...
static const struct usb_device_id asix88179_eth_id_table[] = {
...
U_BOOT_USB_DEVICE(asix88179_eth, asix88179_eth_id_table);
#endif
