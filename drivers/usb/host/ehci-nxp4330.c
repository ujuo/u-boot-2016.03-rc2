...
#if	(0)
#define DBGOUT(msg...)		do { printf("ehci: " msg); } while (0)
#else
#define DBGOUT(msg...)		do {} while (0)
#endif

#define PHY_BASE_ADDR_USB_EHC			(0xC0030000)
...
static int nxp4330_ehci_probe(struct udevice *dev)
{
	struct nxp4330_ehci_priv *priv = dev_get_priv(dev);
	...
	nxp_usb_phy_init(NXP_USB_PHY_EHCI);

	hccr = (struct ehci_hccr *)(priv->hcd_base);
	hcor = (struct ehci_hcor *)
		((u32)hccr + HC_LENGTH(ehci_readl(&hccr->cr_capbase)));

	debug("ehci-nxp4330: init hccr %x and hcor %x hc_length %d\n",
	      (u32)hccr, (u32)hcor,
...
static int nxp4330_ehci_remove(struct udevice *dev)
{
	int ret;
...
static const struct udevice_id ehci_usb_ids[] = {
	{ .compatible = "nexell,nxp4330-ehci", },
	{ }
};

U_BOOT_DRIVER(ehci_nxp4330) = {
	.name	= "ehci_nxp4330",
	.id	= UCLASS_USB,
	.of_match = ehci_usb_ids,
	.probe = nxp4330_ehci_probe,
	.remove = nxp4330_ehci_remove,
	.ops	= &ehci_usb_ops,
	.platdata_auto_alloc_size = sizeof(struct usb_platdata),
	.priv_auto_alloc_size = sizeof(struct nxp4330_ehci_priv),
...
{
	*hccr = (struct ehci_hccr *)(PHY_BASE_ADDR_USB_EHC)I;
