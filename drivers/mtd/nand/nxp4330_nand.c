...
#if	(0)
#define TM_DBGOUT(msg...)		{ printf(KERN_INFO "nand: " msg); }
#else
#define TM_DBGOUT(msg...)		do {} while (0)
#endif

#if	(0)
#define DBGOUT(msg...)		do { printf(KERN_INFO "nand: " msg); } while (0)
#else
#define DBGOUT(msg...)		do {} while (0)
#endif

#define ERROUT(msg...)		{ 					\
		printf(KERN_ERR "ERROR: %s, %s line %d: \n",		\
...
int board_nand_init(struct nand_chip *chip)
{
	...
	*/
	...
	//printf(KERN_INFO "NAND ecc: Hardware (delay %d)\n", chip->chip_delay);
#elif defined (CONFIG_MTD_NAND_ECC_BCH)
	...
	printf(KERN_INFO "NAND ecc: Software BCH %d \n", ECC_BCH_BITS);
#else
	chip->ecc.mode = NAND_ECC_SOFT;
	printf(KERN_INFO "NAND ecc: Software \n");
#endif
...
{
	struct mtd_info  *mtd = &nand_info[dev]; //container_of(chip, struct mtd_info, priv);
	/* Do nothing fot select but set layout */
//	printf("%s chip_addr=%X mtd_addr=%x chip_addr2=%X\n", __func__, chip, mtd, mtd->priv);
