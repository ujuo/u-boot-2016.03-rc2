...
void clock_init_safe(void)
{
	...
	writel(PLL6_CFG_DEFAULT, &ccm->pll6_cfg); /* Settingt PLL6 for 600MHz */
	...
	/* ksw:
		SUN8I has ARISC, and this setting make this clock to 200MHz.,
		Which r_uart should be initialized as 200MHz clock.
		Formerly, it is HOSC frequency which is 24Mhz, It makes confusion to 
		bootloader and kernel serial drivers.
		So I decide to setup APB0/CPUS frequency to 200Mhz in bootloader and SPL.
	*/
	/* APB0 div ratio set to 0 */
	writel(0x00000000, &prcm->apb0_ratio); /* 1/1 */
	/* Now Set CPUS Frequency SRC to PLL6 PREDIV3 */
	writel(0x00020200, &prcm->cpus_cfg);   /* PLL6 PREDIV 3 */
#endif
}
#endif
