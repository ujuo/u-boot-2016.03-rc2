...
#if defined(CONFIG_USB_ETHER_RTL8152) && defined(CONFIG_USB_EHCI_SUNXI)
	sunxi_gpio_set_cfgpin(SUNXI_GPL(4), 1);
	sunxi_gpio_set_drv(SUNXI_GPL(4), 2);
	sunxi_gpio_output(SUNXI_GPL(4), 1);
	
	sunxi_gpio_set_cfgpin(SUNXI_GPL(11), 1);
	sunxi_gpio_set_drv(SUNXI_GPL(11), 2);
	sunxi_gpio_output(SUNXI_GPL(11), 1);
#endif
#if 0
	sunxi_gpio_set_cfgpin(SUNXI_GPL(5), 1);
	sunxi_gpio_set_drv(SUNXI_GPL(5), 2);
	sunxi_gpio_output(SUNXI_GPL(5), 1);
#endif
...
#endif
	setbits_le32(&ccm->nand0_clk_cfg, CCM_NAND_CTRL_ENABLE | AHB_DIV_1);
	setbits_le32(&ccm->ahb_reset0_cfg, CCM_NAND_RST_SET);
}
#endif
...
//	sunxi_ehci_board_init();
//	sunxi_ohci_board_init();
	return 0;
}
#endif
